...
public class MovieProvider extends ContentProvider {
  public static final String AUTHORITY = "net.simonvt.schematic.sample.MovieProvider";

  private static final int LISTS_LISTS = 0;

  private static final UriMatcher MATCHER = new UriMatcher(UriMatcher.NO_MATCH);

  static {
    MATCHER.addURI(AUTHORITY, "lists", LISTS_LISTS);
  }
  ...
  public int bulkInsert(Uri uri, ContentValues[] values) {
    ...
    try {
      switch(MATCHER.match(uri)) {
        case LISTS_LISTS: {
          long[] ids = insertValues(db, "popular_movies", values);
  ...
  public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
    ...
    switch(MATCHER.match(uri)) {
      case LISTS_LISTS: {
        SelectionBuilder builder = getBuilder("Lists");
        if (sortOrder == null) {
          sortOrder = "title ASC";
        }
        String table = "popular_movies";
  ...
  public Uri insert(Uri uri, ContentValues values) {
    ...
    switch(MATCHER.match(uri)) {
      case LISTS_LISTS: {
        final long id = db.insertOrThrow("popular_movies", null, values);
        getContext().getContentResolver().notifyChange(uri, null);
  ...
  public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
    ...
    switch(MATCHER.match(uri)) {
      case LISTS_LISTS: {
        ...
        final int count = builder.table("popular_movies")
            .update(db, values);
        if (count > 0) {
          getContext().getContentResolver().notifyChange(uri, null);
        }
